...
1. **빨래를 세탁 → 건조 → 개기 순으로 한 사람이 다 하면 느린데, 세 명이 컨베이어처럼 나눠 맡으면 왜 훨씬 많이 처리돼?** 첫 빨래가 끝나는 시간은 똑같은데 뭐가 빨라진 거야? (힌트: 한 벌이 끝나는 시간(latency)과 시간당 빨래 수(throughput)를 구분해보기)
2. **앞 명령이 방금 계산한 답을 뒤 명령이 바로 써야 하는데 그 답이 아직 "적히는 중"이면?** CPU는 어떻게 다 적히기 전에 결과를 옆으로 몰래 빼돌려서(forwarding) 안 기다리게 해? (힌트: 요리사가 접시에 담기 전에 바로 다음 사람 손에 건네주는 것)
3. **파이프라인 단계를 잘게 쪼갤수록 좋을 것 같은데, 왜 31단계까지 늘렸던 펜티엄 4는 오히려 실패했어?** (힌트: 갈림길에서 예측을 틀리면 이미 만들던 걸 전부 버려야 하는데, 단계가 깊을수록 버려야 하는 양도 커진다는 점)
