...
1. **일의 95%를 여럿이 나눠 할 수 있어도, 사람을 100명 1000명 아무리 늘려도 20배보다 빨라지지 못한대(암달의 법칙).** 나머지 5%가 뭘 하길래 발목을 잡아? (힌트: 혼자만 해야 하는 5%는 사람이 아무리 많아도 그대로 남아서, 그게 넘을 수 없는 바닥(천장)이 된다는 점)
2. **코드를 빠르게 하려면 왜 "어디가 느린지 먼저 재보기(측정)"부터 해야 해?** 눈에 보이는 데부터 다 고치면 안 돼? (힌트: 전체 시간의 1%인 곳을 2배 빠르게 해봤자 전체는 0.5%만 줄고, 90%를 잡아먹는 한 곳을 찾아야 크게 준다는 점)
3. **우리 서버가 "평균 10ms"인데 왜 어떤 손님은 1초씩 기다린다고 화를 내?** 평균만 보면 왜 속아? (힌트: 100명 중 99명이 빨라도 1명이 아주 느리면 평균엔 안 드러나는 "꼬리(p99)"를 봐야 한다는 점)
