...
1. **도서관에서 책을 "아무 칸에나 꽂기"와 "딱 한 칸에만 꽂기" 중간쯤인 "이 8칸 중 아무 데나"(set-associative) 규칙을 왜 제일 많이 써?** (힌트: 아무 데나 = 찾을 때 전부 뒤져야 해서 느림, 한 칸만 = 인기 책 둘이 같은 칸을 두고 싸움 — 둘의 절충)
2. **두 사람이 서로 다른 칸(`counter_a`, `counter_b`)에 각자 숫자를 쓰는데, 두 칸이 같은 64칸짜리 상자(캐시 라인) 안에 있으면 왜 서로 방해해서 수십 배 느려져(false sharing)?** (힌트: 캐시는 상자를 통째로 주고받아서, 남이 같은 상자의 다른 칸만 건드려도 내 상자를 통째로 빼앗김)
3. **캐시에서 숫자를 고칠 때 왜 메모리에 바로 안 쓰고 "쫓겨날 때 몰아서"(write-back) 써?** 바로 쓰면 안전하고 편하지 않아? (힌트: 같은 값을 100번 고치는데 매번 느린 메모리까지 갔다 오면 얼마나 손해인지)
