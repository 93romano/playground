...
1. **ISA가 하드웨어와 소프트웨어 사이의 "계약"이라는데, 그게 있으면 뭐가 좋아?** 인텔 CPU와 애플 CPU는 속이 완전히 다른데 왜 똑같은 게임이 (거의) 다 돌아가? (힌트: 콘센트 모양만 약속해두면 발전소가 어떻게 전기를 만들든 상관없이 플러그를 꽂을 수 있는 것)
2. **컴퓨터는 "해야 할 일(명령)"과 "재료(데이터)"를 왜 같은 서랍(메모리)에 섞어서 보관해?** 섞으면 헷갈리지 않아? (힌트: 명령도 결국 숫자라서, 한 프로그램이 다른 프로그램을 데이터처럼 읽고 고칠 수 있는 유연함 — 폰 노이만)
3. **프로그램이 파일을 저장하려면 왜 직접 못 하고 꼭 "운영체제님 해주세요"라고 부탁(시스템 콜)해야 해?** 그리고 그 부탁 한 번이 왜 그렇게 느려? (힌트: 아무나 하드웨어를 직접 만지면 위험하니 문지기(Ring 0/3)를 두는데, 그 문을 넘을 때마다 드는 채비 비용)
