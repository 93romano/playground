...
1. **레시피에 "1. 물 끓이기(10분) 2. 계란 까기"가 있으면 물 끓는 동안 멍하니 기다리지 않고 계란부터 까잖아.** CPU도 느린 메모리를 기다리는 동안 뒤 명령을 먼저 한대(OoO). 근데 순서를 막 바꾸면 요리(결과)가 안 망가져? (힌트: 서로 상관없는 일만 앞당기고, 접시에 낼 때는 원래 순서대로 낸다는 점)
2. **종이의 "A" 칸을 지우고 다시 쓰려면 기다려야 하는데, 그냥 새 종이 칸을 꺼내 쓰면 안 기다려도 되잖아.** CPU가 레지스터 이름(r1)을 몰래 다른 물리 칸으로 바꿔치기하는 게 왜 빨라져? (힌트: 이름은 같은 r1이지만 사실 서로 다른 값이라 붙어있을 이유가 없는 "가짜 의존")
3. **CPU는 갈림길 결과가 나오기도 전에 "아마 왼쪽이겠지" 하고 미리 달려가 일을 해둔대(투기 실행).** 맞으면 이득, 틀리면 어떻게 되고, 왜 이 습관이 나중에 보안 구멍(Spectre)이 됐어? (힌트: 틀려서 결과는 되돌려도 이미 캐시에 남긴 흔적은 안 지워진다는 점 — 12장으로 이어짐)
