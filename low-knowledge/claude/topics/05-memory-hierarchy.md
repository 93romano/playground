...
1. **왜 컴퓨터는 메모리를 한 종류로 안 만들고, 빠르고 작은 것(레지스터·L1)부터 느리고 큰 것(DRAM·SSD)까지 여러 층으로 쌓아?** 그냥 다 제일 빠른 걸로 하면 안 돼? (힌트: 제일 빠른 메모리는 비트 하나에 트랜지스터가 6개나 들어 비싸고 자리를 많이 차지한다는 점)
2. **숫자 하나만 달라고 했는데 컴퓨터는 그 옆 64칸(캐시 라인)을 통째로 가져와.** 이 "낭비"가 왜 사실은 이득이야? (힌트: 방금 쓴 물건은 곧 또 쓰고(시간 지역성), 그 옆 물건도 곧 쓴다(공간 지역성)는 습관)
3. **책을 1쪽→2쪽 순서대로 읽는 것과, "다음은 87쪽 봐, 거기서 또 다음 쪽을 알려줄게"처럼 뛰어다니며 읽는 것 중 왜 뒤엣것이 100배 느려?** 읽는 글자 수는 똑같은데. (힌트: 다음 주소를 알려면 지금 것을 먼저 다 읽어야 해서 기다림을 겹쳐 숨길 수 없는 pointer chasing)
