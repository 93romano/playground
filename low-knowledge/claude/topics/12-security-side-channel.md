...
1. **비밀번호를 한 글자씩 비교하다 틀리면 바로 멈추는 코드가 왜 위험해?** 해커가 "얼마나 빨리 틀렸는지" 시간만 재도 어떻게 비밀번호를 한 글자씩 알아내(timing 공격)? (힌트: 앞 글자가 맞을수록 조금 더 오래 비교하니까, 걸린 시간이 길어질수록 앞부분을 맞혔다는 뜻)
2. **CPU가 갈림길에서 미리 달려가 한 일을 "틀렸다"며 되돌려도, 왜 캐시에는 흔적이 남아 비밀이 새(Spectre)?** 되돌렸으면 없던 일 아냐? (힌트: 계산 결과(레지스터)는 지웠지만 그 과정에서 "무엇을 캐시에 데워놨는지"는 안 지워져서, 나중에 접근 시간으로 알아낼 수 있다는 점 — 4장 투기 실행의 어두운 면)
3. **속도를 위한 CPU의 똑똑한 습관(미리 예측·캐시 공유)이 왜 하필 보안 구멍이 돼?** 그래서 암호 코드는 왜 일부러 "비밀이 뭐든 항상 똑같은 시간"이 걸리게(constant-time) 짜? (힌트: 비밀에 따라 빨라지거나 느려지는 순간 그 차이 자체가 정보를 흘리는 통로가 된다는 점)
